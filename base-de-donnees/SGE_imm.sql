/*
  SGE_imm.sql
  Interface machine-machine de base: vues et routines CRUD simples.
...
SELECT p.id_produit, p.nom, p.type, p.description, p.marque, p.modele, p.fournisseur
FROM produit p;
...
CREATE OR REPLACE FUNCTION imm_supprimer_personnel(p_id integer)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION imm_supprimer_produit(p_id integer)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION imm_ajouter_lot(p_id_produit varchar(7), p_quantite dom_quantite)
RETURNS integer
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION imm_supprimer_colis(p_no_colis integer)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
